...
-- Function: "FormBuilder".refresh_question_ts_data()

-- DROP FUNCTION "FormBuilder".refresh_question_ts_data();
...
begin
	...
	FOR q IN SELECT qe.id as id, qe.short_name as short_name, qe.description as description, fe.learn_more as learn_more FROM "FormBuilder".form_element fe inner join "FormBuilder".question qe on fe.id = qe.parent_id inner join "FormBuilder".form frm on fe.form_id=frm.id WHERE fe.id = fid LOOP
		data = data ||
