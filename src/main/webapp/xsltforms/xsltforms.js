...
var xforms = {
	...
	profiling : function() {
		...
		s += "\nHTML Elements: " + hc + " Time: " + this.htmltime + "ms";
...
MIPBinding.prototype.evaluate = function(ctx) {
	var deps = this.depsElements;
	var depsN = this.depsNodes;
	...
	if (build) {
		...
		depsN.length = 0;
		deps.length = 0;
		return this.binding.evaluate(ctx.node, this.depsNodes, null, this.depsElements);
	} else {
...
XFInstance.prototype.validate_ = function(node, readonly, notrelevant) {
	...
	if (bindids) {
		...
		for (var i = 0, len = binds.length; i < len; i++) {
			...
			if (bind.required) {
				this.setProperty_(node, "required", bind.required.evaluate(ctx));
			}
			if (notrelevant || !relevantfound || bind.relevant != undefined) {
				this.setProperty_(node, "notrelevant", notrelevant || !(bind.relevant? bind.relevant.evaluate(ctx) : true));
				relevantfound = relevantfound || bind.relevant != undefined;
			}
			if (readonly || !readonlyfound || bind.readonly != undefined) {
				this.setProperty_(node, "readonly", readonly || (bind.readonly? bind.readonly.evaluate(ctx) : bind.calculate ? true : false));
			...
			this.setProperty_(node, "notvalid",
				...
				&& (!bind.constraint || bind.constraint.evaluate(ctx))));
...
function XFSetvalue(binding, value, literal, context, ifexpr, whileexpr) {
	this.binding = binding;
	...
	try {
		...
		if ((res instanceof Array) && res.length > 1) {
			...
			for (var i = 0, len = res.length; i < len; i++) {
				posres.push({count: Core.selectNodesLength("preceding::node() | ancestor::node()", res[i]), node: res[i]});
			}
