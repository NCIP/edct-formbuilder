...
XsltForms_browser.dialog = {
	...
	//TOLA 10/17/2012 Added Calendar-related changes: Calendar not popping up in correct position
	show : function(div, parent, modal, coord) {
			...
			if(coord) {
				Core.setPos(div, coord.x, coord.y);
			}
			else{
				if (parent) {
...
XsltForms_browser.i18n = {
	...
	// TOLA 10/17/2012 Allow decimal point in number field
	get : function(key) {
		...
		/*if (XsltForms_globals.language === "navigator" || XsltForms_globals.language !== XsltForms_browser.selectSingleNodeText('language', XsltForms_browser.config)) {
			...
			if (found) {*/
				XsltForms_browser.loadProperties("config_en_us.xsl");
...
XsltForms_input.prototype.initInput = function(type) {
	...
	if (input.type === "password") {
		...
		if (tclass === "boolean") {
			...
			// TOLA 10/17/2012 Fixed right-side positioning of the cursor
...
XsltForms_itemset.prototype.refresh_ = function(element, cont) {
	...
	if (nodeLabel) {
		...
		try {
			var maxLettersInOptionText = 50; 
			var text = XsltForms_browser.getValue(nodeLabel, true);
			if(!!text && text.length > maxLettersInOptionText) {
				element.setAttribute("title", text);
				text = text.substring(0,maxLettersInOptionText) + '...';
			}
			element.text = text;
		} catch(e) {
		}
...
//TOLA 10/17/2012 Fix to allow correct handling of custom types
XsltForms_unionType.prototype.addType = function(type) {
	var listTypes = memberTypes ? memberTypes.split(' ') : [];
	for ( var i=0; i < listTypes.length; ++i ) {
		var t = listTypes[ i ];
		this.baseTypes.push(typeof t === "string"? this.schema.getType(t) : t);
	}
	return this;
};
